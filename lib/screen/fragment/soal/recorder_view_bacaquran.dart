...
class RecorderBacaQuranView extends StatefulWidget {
  final Function onSaved;
  final Function onDuplicate;
  final int number;

  const RecorderBacaQuranView(
      {Key key, @required this.onSaved, this.onDuplicate, this.number})
      : super(key: key);
  @override
  _RecorderBacaQuranViewState createState() => _RecorderBacaQuranViewState();
}
...
class _RecorderBacaQuranViewState extends State<RecorderBacaQuranView> {
  ...
  _stopRecording() async {
    ...
    var request = http.MultipartRequest(
      'POST',
      Uri.parse("${Paths.BASEURL}${Paths.BACAQURAN_UPDATEJAWABAN}"),
    );
    ...
    if (response.statusCode == 200) {
      ...
      return widget.onSaved('bacaQuran' +
          GetStorage().read(ID_MURID).toString() +
          filePath.split("/").last);
    }
  }
